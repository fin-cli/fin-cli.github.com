...
[FIN-CLI](https://fin-cli.org/) は [FinPress](https://finpress.org/) を管理するためのコマンドラインインターフェースです。
...
現在の安定バージョンは [2.12.0](https://make.finpress.org/cli/2025/05/07/fin-cli-v2-12-0-release-notes/) です。 最新情報を得たい人は、[@fincli on Twitter](https://twitter.com/fincli) をフォローするか、[メーリングリストにサインアップ](https://make.finpress.org/cli/subscribe/)してください。[ロードマップ](https://make.finpress.org/cli/handbook/roadmap/)で今後のリリース予定を知ることができます。
...
FIN-CLI は、みなさんが FinPress の管理画面でやりたいと思っていることに対するコマンドラインインターフェースを提供します。
たとえば、`fin plugin install --activate` ([ドキュメント](https://developer.finpress.org/cli/commands/plugin/install/)) というコマンドを実行すると、プラグインをインストールした上で有効化できます。
...
さらに FIN-CLI では、FinPress の管理画面ではできない多くのことを行えます。たとえば、`fin transient delete --all` ([ドキュメント](https://developer.finpress.org/cli/commands/transient/delete/)) というコマンドを実行すると、一時的に保存されているすべてのデータが削除されます。
...
FIN-CLI の使い方について詳しく知りたいときは、[クイックスタートガイド](https://make.finpress.org/cli/handbook/quick-start/)を読んでください。[Shell friends](https://make.finpress.org/cli/handbook/shell-friends/) では便利なコマンドラインユーティリティについても学べます。
...
FIN-CLI をインストールする前に、動作環境を確認します。
...
動作条件を再度確認し、`wget` または `curl` で [fin-cli.phar](https://raw.githubusercontent.com/fin-cli/builds/gh-pages/phar/fin-cli.phar) をダウンロードします。
...
FIN-CLI を `fin` コマンドで実行するには、fin-cli.phar が実行可能で、かつ、環境変数 `PATH` に登録された場所に置かれていることが必要です。例を示します。
...
FIN-CLI のインストールが成功していれば、`fin --info` コマンドで以下のように出力されるはずです。
...
FIN-CLI は、`fin cli update` ([ドキュメント](https://developer.finpress.org/cli/commands/cli/update/)) コマンド、または上述のインストール方法を再度行うことでアップデートできます。

ただし、FIN-CLI のオーナーが root または他のシステム管理者の場合は、`sudo fin cli update` と実行する必要があります。

チャレンジ精神旺盛な人は `fin cli update --nightly` を実行してみましょう。最新の開発者向けバージョンの FIN-CLI を使うことができます。開発者向けバージョンは、あなたの開発環境で使用する上で十分な信頼性があり、常に最新の機能を含んでいます。

## タブ補完

FIN-CLI には、Bash および zsh 向けのタブ補完スクリプトがあります。スクリプトを使うときは [fin-completion.bash](https://raw.githubusercontent.com/fin-cli/fin-cli/v2.12.0/utils/fin-completion.bash) をダウンロードして、`~/.bash_profile` で読み込みます。
...
zsh の場合は、`bashcompinit` をロードした後に `fin-completion.bash` を読み込ませる必要があるかもしれません。`.zshrc` に次のコードを追加してください。
...
FIN-CLI のメンテナーとその貢献者が一般的な質問に答えられる時間は限られています。[最新版](https://make.finpress.org/cli/handbook/roadmap/)のみが公式にサポートされるバージョンです。
...
-   [Open or closed issues in the FIN-CLI GitHub organization](https://github.com/issues?q=sort%3Aupdated-desc+org%3Afin-cli+is%3Aissue)
...
-   FinPress サポートフォーラムに[新しいスレッドを投稿](https://finpress.org/support/forum/fin-advanced/#new-post)し、'FIN-CLI' というタグを付けてコミュニティ内で見つけやすくします。
...
それぞれの **コマンド** は、FIN-CLI の関数の一つとして定義されています。`fin plugin install` ([ドキュメント](https://developer.finpress.org/cli/commands/plugin/install/)) はその一つで、`fin plugin activate` ([ドキュメント](https://developer.finpress.org/cli/commands/plugin/activate/)) もまた、その一つです。

FIN-CLI では、さまざまな実行可能なクラス、関数、クロージャをコマンドとして実行できます。コマンドの実行に必要な情報は PHPDoc で記述します。 `FIN_CLI::add_command()` ([ドキュメント](https://make.finpress.org/cli/handbook/internal-api/fin-cli-add-command/)) は、内部コマンドおよびサードパーティコマンドの登録に使われています。
...
FIN-CLI は多くのコマンドで構成され、意外と簡単にカスタムコマンドを作ることができます。詳しくは [commands cookbook](https://make.finpress.org/cli/handbook/commands-cookbook/) を参照してください。[内部 API のドキュメント](https://make.finpress.org/cli/handbook/references/internal-api/)には、カスタムコマンドで使用できるさまざまな便利機能が載っています。

## 貢献

ようこそ ! ありがとう !

私たちは、みなさんが率先して貢献してくださることに感謝します。あなたやあなたのまわりのコミュニティによって、FIN-CLI はすばらしいプロジェクトになります。
...
FIN-CLI にはプロジェクトメンテナーがいます: [schlessera](http://github.com/schlessera) です。
...
[composer.json](https://github.com/fin-cli/fin-cli/blob/master/composer.json) に記載されているライブラリに依存しており、以下のプロジェクトからコードやアイディアを得ています。
